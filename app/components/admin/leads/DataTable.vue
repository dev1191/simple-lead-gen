...
interface DataTableProps {
  ...
  leads: LeadsComposable;
}

const props = defineProps<DataTableProps>();
const { leads, columns } = props;

// Local reactive filter states synced with composable filters
const globalFilter = ref(leads.filters.value.search || "");
const statusFilter = ref(leads.filters.value.status || "");

// Sync local filters → composable filters and fetch leads
watch(globalFilter, (val) => {
  leads.updateSearch(val);
});

watch(statusFilter, (val) => {
  leads.updateFilter("status", val);
});

// Sync composable filters → local filters (in case updated externally)
watch(
  () => leads.filters.value.search,
  (val) => {
...
const tableState = reactive({
  sorting: [
    {
      id: leads.sort.value.field,
      desc: leads.sort.value.order === "desc",
    },
  ],
  pagination: {
    pageIndex: leads.currentPage.value - 1,
    pageSize: leads.pageSize.value,
  },
  globalFilter: globalFilter.value,
});
const data = computed(() => leads.leads.value || []);
...
const table = useVueTable({
  ...
  pageCount: Math.ceil(leads.totalCount.value / leads.pageSize.value),
  onSortingChange: (updater) => {
    const newSorting =
      typeof updater === "function" ? updater(tableState.sorting) : updater;
    if (newSorting.length > 0) {
      const { id, desc } = newSorting[0];
      leads.updateSort(id, desc ? "desc" : "asc");
  ...
  onPaginationChange: (updater) => {
    ...
    leads.changePage(newPagination.pageIndex + 1);
