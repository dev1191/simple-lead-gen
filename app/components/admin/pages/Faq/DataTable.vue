...
interface DataTableProps {
  columns: ColumnDef<Faq, any>[];
  faqs: FaqsComposable;
}

const props = defineProps<DataTableProps>();
const { faqs, columns } = props;

// Local reactive filter states synced with composable filters
const globalFilter = ref(faqs.filters.value.search || "");
const statusFilter = ref(faqs.filters.value.status || "");

// Sync local filters → composable filters and fetch faqs
watch(globalFilter, (val) => {
  faqs.updateSearch(val);
});

watch(statusFilter, (val) => {
  faqs.updateFilter("status", val);
});

// Sync composable filters → local filters (in case updated externally)
watch(
  () => faqs.filters.value.search,
  (val) => {
...
const tableState = reactive({
  sorting: [
    {
      id: faqs.sort.value.field,
      desc: faqs.sort.value.order === "desc",
    },
  ],
  pagination: {
    pageIndex: faqs.currentPage.value - 1,
    pageSize: faqs.pageSize.value,
  },
  globalFilter: globalFilter.value,
});
const data = computed(() => faqs.faqs.value || []);
...
const table = useVueTable({
  ...
  pageCount: Math.ceil(faqs.totalCount.value / faqs.pageSize.value),
  onSortingChange: (updater) => {
    const newSorting =
      typeof updater === "function" ? updater(tableState.sorting) : updater;
    if (newSorting.length > 0) {
      const { id, desc } = newSorting[0];
      faqs.updateSort(id, desc ? "desc" : "asc");
  ...
  onPaginationChange: (updater) => {
    ...
    faqs.changePage(newPagination.pageIndex + 1);
