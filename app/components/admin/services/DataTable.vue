...
interface DataTableProps {
  columns: ColumnDef<Service, any>[];
  services: ServicesComposable;
}

const props = defineProps<DataTableProps>();
const { services, columns } = props;

// Local reactive filter states synced with composable filters
const globalFilter = ref(services.filters.value.search || "");
const statusFilter = ref(services.filters.value.status || "");

// Sync local filters → composable filters and fetch services
watch(globalFilter, (val) => {
  services.updateSearch(val);
});

watch(statusFilter, (val) => {
  services.updateFilter("status", val);
});

// Sync composable filters → local filters (in case updated externally)
watch(
  () => services.filters.value.search,
  (val) => {
...
const tableState = reactive({
  sorting: [
    {
      id: services.sort.value.field,
      desc: services.sort.value.order === "desc",
    },
  ],
  pagination: {
    pageIndex: services.currentPage.value - 1,
    pageSize: services.pageSize.value,
  },
  globalFilter: globalFilter.value,
});
const data = computed(() => services.services.value || []);
...
const table = useVueTable({
  ...
  pageCount: Math.ceil(services.totalCount.value / services.pageSize.value),
  onSortingChange: (updater) => {
    const newSorting =
      typeof updater === "function" ? updater(tableState.sorting) : updater;
    if (newSorting.length > 0) {
      const { id, desc } = newSorting[0];
      services.updateSort(id, desc ? "desc" : "asc");
  ...
  onPaginationChange: (updater) => {
    ...
    services.changePage(newPagination.pageIndex + 1);
