...
interface DataTableProps {
  columns: ColumnDef<Vendor, any>[];
  vendors: VendorsComposable;
}

const props = defineProps<DataTableProps>();
const { vendors, columns } = props;

// Local reactive filter states synced with composable filters
const globalFilter = ref(vendors.filters.value.search || "");
const statusFilter = ref(vendors.filters.value.status || "");

// Sync local filters → composable filters and fetch vendors
watch(globalFilter, (val) => {
  vendors.updateSearch(val);
});

watch(statusFilter, (val) => {
  vendors.updateFilter("status", val);
});

// Sync composable filters → local filters (in case updated externally)
watch(
  () => vendors.filters.value.search,
  (val) => {
...
const tableState = reactive({
  sorting: [
    {
      id: vendors.sort.value.field,
      desc: vendors.sort.value.order === "desc",
    },
  ],
  pagination: {
    pageIndex: vendors.currentPage.value - 1,
    pageSize: vendors.pageSize.value,
  },
  globalFilter: globalFilter.value,
});
const data = computed(() => vendors.vendors.value || []);
...
const table = useVueTable({
  ...
  pageCount: Math.ceil(vendors.totalCount.value / vendors.pageSize.value),
  onSortingChange: (updater) => {
    const newSorting =
      typeof updater === "function" ? updater(tableState.sorting) : updater;
    if (newSorting.length > 0) {
      const { id, desc } = newSorting[0];
      vendors.updateSort(id, desc ? "desc" : "asc");
  ...
  onPaginationChange: (updater) => {
    ...
    vendors.changePage(newPagination.pageIndex + 1);
