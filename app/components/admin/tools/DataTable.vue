...
interface DataTableProps {
  columns: ColumnDef<Tool, any>[];
  tools: ToolsComposable;
}

const props = defineProps<DataTableProps>();
const { tools, columns } = props;

// Local reactive filter states synced with composable filters
const globalFilter = ref(tools.filters.value.search || "");
const statusFilter = ref(tools.filters.value.status || "");

// Sync local filters → composable filters and fetch tools
watch(globalFilter, (val) => {
  tools.updateSearch(val);
});

watch(statusFilter, (val) => {
  tools.updateFilter("status", val);
});

// Sync composable filters → local filters (in case updated externally)
watch(
  () => tools.filters.value.search,
  (val) => {
...
const tableState = reactive({
  sorting: [
    {
      id: tools.sort.value.field,
      desc: tools.sort.value.order === "desc",
    },
  ],
  pagination: {
    pageIndex: tools.currentPage.value - 1,
    pageSize: tools.pageSize.value,
  },
  globalFilter: globalFilter.value,
});
const data = computed(() => tools.tools.value || []);
...
const table = useVueTable({
  ...
  pageCount: Math.ceil(tools.totalCount.value / tools.pageSize.value),
  onSortingChange: (updater) => {
    const newSorting =
      typeof updater === "function" ? updater(tableState.sorting) : updater;
    if (newSorting.length > 0) {
      const { id, desc } = newSorting[0];
      tools.updateSort(id, desc ? "desc" : "asc");
  ...
  onPaginationChange: (updater) => {
    ...
    tools.changePage(newPagination.pageIndex + 1);
